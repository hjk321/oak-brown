...
const u8 sDexScreen_ShinyIcon[] = INCBIN_U8("graphics/pokedex/shiny_marker.4bpp");
...
static void ItemPrintFunc_OrderedListMenu(u8 windowId, s32 itemId, u8 y)
{
    ...
    if (caught)
    {
        if (DexScreen_GetSetPokedexFlag(species, FLAG_GET_SHINY, TRUE))
            BlitMoveInfoIcon(sPokedexScreenData->numericalOrderWindowId, 30, 0x28, y);
        else
...
{
    return GenerateDexMonPersonality(species, DexScreen_GetSetPokedexFlag(species, FLAG_GET_SHINY, TRUE));
}
...
bool8 DexScreen_DrawMonPicInCategoryPage(u16 species, u8 slot, u8 numSlots)
{
    ...
    if (sPokedexScreenData->categoryMonInfoWindowIds[slot] == 0xFF)
    {
        if (species != SPECIES_NONE)
        {
            ...
            {
                if (DexScreen_GetSetPokedexFlag(species, FLAG_GET_SHINY, TRUE))
                    BlitBitmapRectToWindow(sPokedexScreenData->categoryMonInfoWindowIds[slot], sDexScreen_ShinyIcon, 0, 0, 8, 8, 2, 3, 8, 8);
                else
